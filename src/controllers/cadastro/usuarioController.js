...
async function adicionar(request, response) {
    ...
    try {
        ...
        const errorMessage = error.message.includes('Query não fornecida para logging')
            ...
            : `Erro ao adicionar Usuario: ${error.message}`;
...
async function deleteUsuario(request, response) {
    ...
    try {
        ...
        if (id_usuario) {
            ...
            if (result.rowsAffected[0] > 0) {
                logQuery('info', `O usuário ${id_usuario} deletou o Usuario`, 'sucesso', 'DELETE', id_cliente, id_usuario, query, params);
                response.status(200).json(result.recordset);
            } else {
                //throw new Error(`Erro ao excluir: ${id_usuario} não encontrado.`);
                logQuery('error', `Erro ao excluir: ${id_usuario} não encontrado.`, 'erro', 'DELETE', id_cliente, id_usuario, query, params);

...
// Função assíncrona para obter o próximo ID disponível para um item na tabela DM_itens
...
async function adicionar(request, response) {
  ...
  try {
    ...
    transaction = new sql.Transaction(); // Cria uma nova transação do tipo `sql.Transaction`.
    await transaction.begin(); // Inicia a transação no banco de dados.

    const sqlRequest = new sql.Request(transaction); // Cria uma nova requisição SQL associada à transação.

    sqlRequest.input("ID_Cliente", sql.Int, IDcliente); // Define o parâmetro `ID_Cliente` para a consulta SQL com o tipo `Int` e o valor de `IDcliente`.
    sqlRequest.input("Numero", sql.VarChar, Numero); // Define o parâmetro `Numero` para a consulta SQL com o tipo `VarChar` e o valor de `Numero`.
    sqlRequest.input("Devolucao", sql.Bit, Devolucao); // Define o parâmetro `Devolucao` para a consulta SQL com o tipo `Bit` e o valor de `Devolucao`.
    sqlRequest.input("Identificacao", sql.NVarChar, Identificacao); // Define o parâmetro `Identificacao` para a consulta SQL com o tipo `NVarChar` e o valor de `Identificacao`.
    sqlRequest.input("ChaveAPI", sql.NVarChar, ChaveAPI); // Define o parâmetro `ChaveAPI` para a consulta SQL com o tipo `NVarChar` e o valor de `ChaveAPI`.
    sqlRequest.input("Ativo", sql.Bit, Ativo); // Define o parâmetro `Ativo` para a consulta SQL com o tipo `Bit` e o valor de `Ativo`.
    sqlRequest.input("Deleted", sql.Bit, false); // Define o parâmetro `Deleted` para a consulta SQL com o tipo `Bit` e o valor `false` (não deletado).
    sqlRequest.input("Created", sql.DateTime, new Date()); // Define o parâmetro `Created` para a consulta SQL com o tipo `DateTime` e o valor da data atual (`new Date()`).
    sqlRequest.input("Updated", sql.DateTime, "1900-01-01 00:00:00.000"); // Define o parâmetro `Updated` com um valor padrão de "1900-01-01".
    sqlRequest.input("Versao", sql.Int, Versao || 1); // Define o parâmetro `Versao` para a consulta SQL com o tipo `Int` e o valor de `Versao`, ou `1` se `Versao` não estiver definido.
    sqlRequest.input("Enviada", sql.Bit, Enviada); // Define o parâmetro `Enviada` para a consulta SQL com o tipo `Bit` e o valor de `Enviada`.
    sqlRequest.input("OP_Senha", sql.Bit, OP_Senha); // Define o parâmetro `OP_Senha` para a consulta SQL com o tipo `Bit` e o valor de `OP_Senha`.
    sqlRequest.input("OP_Biometria", sql.Bit, OP_Biometria); // Define o parâmetro `OP_Biometria` para a consulta SQL com o tipo `Bit` e o valor de `OP_Biometria`.
    sqlRequest.input("OP_Facial", sql.Bit, OP_Facial); // Define o parâmetro `OP_Facial` para a consulta SQL com o tipo `Bit` e o valor de `OP_Facial`.
    sqlRequest.input("Integracao", sql.Bit, Integracao); // Define o parâmetro `Integracao` para a consulta SQL com o tipo `Bit` e o valor de `Integracao`.
    sqlRequest.input("ClienteID", sql.VarChar, ClienteID); // Define o parâmetro `ClienteID` para a consulta SQL com o tipo `VarChar` e o valor de `ClienteID`.
    sqlRequest.input("ClienteNome", sql.NVarChar, ClienteNome); // Define o parâmetro `ClienteNome` para a consulta SQL com o tipo `NVarChar` e o valor de `ClienteNome`.
    sqlRequest.input("UserID", sql.VarChar, UserID); // Define o parâmetro `UserID` para a consulta SQL com o tipo `VarChar` e o valor de `UserID`.
    sqlRequest.input("Chave", sql.NVarChar, Chave); // Define o parâmetro `Chave` para a consulta SQL com o tipo `NVarChar` e o valor de `Chave`.
    sqlRequest.input("ID_CR_Usuario", sql.Int, id_usuario); // Define o parâmetro `ID_CR_Usuario` para a consulta SQL com o tipo `Int` e o valor de `id_usuario`.
    sqlRequest.input("Sincronizado", sql.Int, 0); // Define o parâmetro `Sincronizado` como `0` para indicar que o dado ainda não foi sincronizado.
...
async function atualizar(request, response) {
  ...
  try {
    ...
    const requestDM = new sql.Request(transaction); // Cria uma nova requisição SQL associada à transação iniciada anteriormente, permitindo que os parâmetros e consultas sejam executados dentro da transação.

    requestDM.input("ID_DM", sql.Int, ID_DM); // Define o parâmetro `ID_DM` com tipo `Int` e o valor de `ID_DM`, identificador do DM.
    requestDM.input("ID_Cliente", sql.Int, IDcliente); // Define o parâmetro `ID_Cliente` com tipo `Int` e o valor de `IDcliente`, identificador do cliente.
    requestDM.input("Numero", sql.VarChar, Numero); // Define o parâmetro `Numero` com tipo `VarChar` e o valor de `Numero`, número associado ao cliente.
    requestDM.input("Identificacao", sql.NVarChar, Identificacao); // Define o parâmetro `Identificacao` com tipo `NVarChar` e o valor de `Identificacao`, identificador do cliente.
    requestDM.input("Ativo", sql.Bit, Ativo); // Define o parâmetro `Ativo` com tipo `Bit` e o valor de `Ativo`, status de ativação.
    requestDM.input("Deleted", sql.Bit, Deleted); // Define o parâmetro `Deleted` com tipo `Bit` e o valor de `Deleted`, se o registro foi deletado.
    requestDM.input("Created", sql.DateTime, Created); // Define o parâmetro `Created` com tipo `DateTime` e o valor de `Created`, data de criação do registro.
    requestDM.input("Updated", sql.DateTime, Updated || new Date()); // Define o parâmetro `Updated` com tipo `DateTime` e o valor de `Updated` (ou o valor atual caso `Updated` seja undefined).
    requestDM.input("Versao", sql.Int, Versao); // Define o parâmetro `Versao` com tipo `Int` e o valor de `Versao`, versão do registro.
    requestDM.input("Enviada", sql.Bit, Enviada); // Define o parâmetro `Enviada` com tipo `Bit` e o valor de `Enviada`, se a informação foi enviada.
    requestDM.input("OP_Senha", sql.Bit, OP_Senha); // Define o parâmetro `OP_Senha` com tipo `Bit` e o valor de `OP_Senha`, se a opção de senha está ativada.
    requestDM.input("OP_Biometria", sql.Bit, OP_Biometria); // Define o parâmetro `OP_Biometria` com tipo `Bit` e o valor de `OP_Biometria`, se a opção de biometria está ativada.
    requestDM.input("OP_Facial", sql.Bit, OP_Facial); // Define o parâmetro `OP_Facial` com tipo `Bit` e o valor de `OP_Facial`, se a opção facial está ativada.
    requestDM.input("Integracao", sql.Bit, Integracao); // Define o parâmetro `Integracao` com tipo `Bit` e o valor de `Integracao`, se a integração está ativada.
    requestDM.input("ClienteID", sql.VarChar, ClienteID); // Define o parâmetro `ClienteID` com tipo `VarChar` e o valor de `ClienteID`, ID do cliente.
    requestDM.input("ClienteNome", sql.NVarChar, ClienteNome); // Define o parâmetro `ClienteNome` com tipo `NVarChar` e o valor de `ClienteNome`, nome do cliente.
    requestDM.input("Chave", sql.NVarChar, Chave); // Define o parâmetro `Chave` com tipo `NVarChar` e o valor de `Chave`, chave associada ao cliente.
    requestDM.input("ChaveAPI", sql.NVarChar, ChaveAPI); // Define o parâmetro `ChaveAPI` com tipo `NVarChar` e o valor de `ChaveAPI`, chave da API utilizada.
    requestDM.input("Devolucao", sql.Bit, Devolucao); // Define o parâmetro `Devolucao` com tipo `Bit` e o valor de `Devolucao`, status de devolução.

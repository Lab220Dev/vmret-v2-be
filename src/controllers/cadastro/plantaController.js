...
async function listarPaginado(request, response) {
  ...
  try {
    ...
    sqlRequest.input("id_cliente", sql.Int, id_cliente); // Adiciona o parâmetro 'id_cliente' na consulta SQL

    if (filters.nome) {
      // Verifica se o filtro 'nome' foi enviado na requisição
      query += ` AND nome LIKE @nome`; // Adiciona uma condição de filtro 'nome' à query
      sqlRequest.input("nome", sql.NVarChar, `%${filters.nome.value}%`); // Adiciona o filtro 'nome' na consulta SQL, permitindo correspondência parcial
    }
    if (filters.local) {
      // Verifica se o filtro 'local' foi enviado na requisição
      query += ` AND id_planta LIKE @id_planta`; // Adiciona uma condição de filtro 'id_planta' à query
      sqlRequest.input("id_planta", sql.Int, `%${filters.id_planta.value}%`); // Adiciona o filtro 'id_planta' na consulta SQL, permitindo correspondência parcial
    }

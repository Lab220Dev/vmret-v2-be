...
async function adicionar(request, response) {
    ...
    try {
        ...
        if (result.rowsAffected[0] > 0) {
            logQuery('info', `Usuário ${id_usuario} criou um novo usuario`, 'sucesso', 'INSERT', id_cliente, id_usuario, query, params);  // Registra no log.
            response.status(201).send('usuario criado com sucesso!');  // Resposta de sucesso.
        } else {
            logQuery('error', `Usuário ${id_usuario} falhou ao criar usuario`, 'falha', 'INSERT', id_cliente, id_usuario, query, params);  // Registra no log.
...
async function deletar(request, response) {
    ...
    try {
        ...
        if (result.rowsAffected[0] > 0) {
            ...
            response.status(400).send('Nenhuma alteração foi feita no centro de custo.');  // Retorna erro se nada foi alterado.
        }
    } catch (error) {
        // Em caso de erro, retorna status 500 e a mensagem de erro.

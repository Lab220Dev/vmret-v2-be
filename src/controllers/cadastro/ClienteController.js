...
async function adicionar(request, response) {
    ...
    try {
        ...
        if (result.rowsAffected[0] > 0) {
            logQuery('info', `Usuário ${id_usuario} criou um novo Cliente`, 'sucesso', 'INSERT', id_cliente, id_usuario, query, params);
            response.status(201).send('Cliente criado com sucesso!');
        } else {
            logQuery('error', `Usuário ${id_usuario} falhou ao criar Cliente`, 'falha', 'INSERT', id_cliente, id_usuario, query, params);
            response.status(400).send('Falha ao criar o Cliente');
        }
...
async function atualizar(request, response) {
    ...
    try {
        ...
        if (result.rowsAffected && result.rowsAffected[0] > 0) {
            ...
            logQuery('error', `Usuário ${id_usuario} tentou atualizar o Cliente ${id_cliente}, mas sem sucesso.`, 'Falha', 'UPDATE', id_cliente, id_usuario, query, params);
...
async function deletar(request, response) {
    ...
    try {
        ...
        if (result.rowsAffected[0] > 0) {
            ...
            logQuery('error', `Erro ao excluir: ${id_cliente} não encontrado.`, 'erro', 'DELETE', id_cliente, id_usuario, query, params);
